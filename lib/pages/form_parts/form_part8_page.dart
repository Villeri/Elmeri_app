...
import 'form_part9_page.dart';

class FormPartEightPage extends StatefulWidget {
  String currentRoom;
  ...
  @override
  State<FormPartEightPage> createState() => _FormPartEightPageState();
}

class _FormPartEightPageState extends State<FormPartEightPage> {
  int thingsOk8 = 0;
  int thingsNotOk8 = 0;
  TextEditingController problemEightController = TextEditingController();
      ...
      body: SingleChildScrollView(
        child: Center(
          child: Column(
            children: [
              FormCardWidget(
                headerText: "8. Kulkuteiden ja lattioiden järjestys",
                thingsOk: thingsOk8.toString(),
                thingsNotOk: thingsNotOk8.toString(),
                plusThingsOk: plusThingsOkPartEight,
                minusThingsOk: minusThingsOkPartEight,
                plusThingsNotOk: plusThingsNotOkPartEight,
                minusThingsNotOk: minusThingsNotOkPartEight,
              ),
                ...
                child: Column(
                  children: [
                    ...
                    ProblemCardProblemsWidget(
                      controller: problemEightController,
                    ),
                    const SizedBox(height: 15),
                    const Text(
                      "Vastuutahot:",
                    ...
                    SaveProblemButtonWidget(
                      onPressed: file != null
                          ? () {
                              ...
                              setState(() {
                                problemsPartEight
                                    .add(problemEightController.text);
                                accountablePeoplePartEight
                                    .add(accountablePeoplePartEight);
                                urgenciesPartEight.add(currentUrgency);
                                problemCardsPartEight.add(
                                  Card(
                                    child: Column(
                                      children: [
                                        Text(
                                            "Poikkeama: ${problemEightController.text}",
                                            style:
              ...
              NextButton(onPressed: () {
                Navigator.push(
                    context,
                    MaterialPageRoute(
                        builder: (context) => FormPartNinePage(
                              ...
                              accountablePeoplePartEight:
                                  accountablePeoplePartEight,
                              imgUrlsPartEight: imgUrlsProblemsPartEight,
                              problemCardsPartEight: problemCardsPartEight,
  ...
  plusThingsOkPartEight() {
    setState(() {
      thingsOk8++;
    });
  }

  minusThingsOkPartEight() {
    if (thingsOk8 > 0) {
      setState(() {
        thingsOk8--;
      });
  ...
  plusThingsNotOkPartEight() {
    setState(() {
      thingsNotOk8++;
    });
  }

  minusThingsNotOkPartEight() {
    if (thingsNotOk8 > 0) {
      setState(() {
        thingsNotOk8--;
      });
  ...
  Future saveDataToDatabase() async {
    if (file == null) return;
    final fileName = "part8problem$problem8Id";
    final destination = "files/$fileName";
    _problemData.problem = problemEightController.text.trim();
    _problemData.accountablePeople = problemsAccountablePeople;
    _problemData.urgency = currentUrgency;
    _problemData.problemId = problem8Id.toString();
    ...
    setState(() {
      problem8Id++;
      imgUrlsProblemsPartEight.add(urlDownload);
    });
  }
