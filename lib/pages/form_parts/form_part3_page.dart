...
import 'form_part4_page.dart';

class FormPartThreePage extends StatefulWidget {
  String currentRoom;
  ...
  @override
  State<FormPartThreePage> createState() => _FormPartThreePageState();
}

class _FormPartThreePageState extends State<FormPartThreePage> {
  TextEditingController problemThreeController = TextEditingController();
      ...
      body: SingleChildScrollView(
        child: Center(
          child: Column(
            children: [
              FormCardWidget(
                headerText: "3. Työpisteiden ja -välinen ergonomia",
                thingsOk: widget.thingsOk3.toString(),
                thingsNotOk: widget.thingsNotOk3.toString(),
                plusThingsOk: plusthingsOkPartThree,
                minusThingsOk: minusthingsOkPartThree,
                plusThingsNotOk: plusThingsNotOkPartThree,
                minusThingsNotOk: minusThingsNotOkPartThree,
              ),
                ...
                child: Column(
                  children: [
                    ...
                    ProblemCardProblemsWidget(
                      controller: problemThreeController,
                    ),
                    const SizedBox(height: 15),
                    const Text(
                      "Vastuutahot:",
                    ...
                    SaveProblemButtonWidget(
                      onPressed: file != null
                          ? () {
                              saveDataToDatabase();
                              setState(() {
                                widget.problemsPartThree.add(
                                    "${widget.currentRoom} ${problemThreeController.text}");
                                widget.accountablePeoplePartThree
                                    .add(problemsAccountablePeople);
                                widget.urgenciesPartThree.add(currentUrgency);
                                widget.problemCardsPartThree.add(
                                  Card(
                                    child: Column(
                                      children: [
                                        Text(
                                            "${widget.currentRoom} Poikkeama: ${problemThreeController.text}",
                                            style:
              ...
              NextButton(onPressed: () {
                Navigator.push(
                    context,
                    MaterialPageRoute(
                        builder: (context) => FormPartFourPage(
  ...
  plusthingsOkPartThree() {
    setState(() {
      widget.thingsOk3++;
    });
  }

  minusthingsOkPartThree() {
    if (widget.thingsOk3 > 0) {
      setState(() {
        widget.thingsOk3--;
      });
    }
  ...
  plusThingsNotOkPartThree() {
    setState(() {
      widget.thingsNotOk3++;
    });
  }

  minusThingsNotOkPartThree() {
    if (widget.thingsNotOk3 > 0) {
      setState(() {
        widget.thingsNotOk3--;
      });
    }
  ...
  Future saveDataToDatabase() async {
    if (file == null) return;
    final fileName = "part3problem$problem3Id";
    final destination = "files/$fileName";
    _problemData.problem =
        "${widget.currentRoom} ${problemThreeController.text.trim()}";
    _problemData.accountablePeople = problemsAccountablePeople;
    _problemData.urgency = currentUrgency;
    _problemData.problemId = problem3Id.toString();
    ...
    setState(() {
      problem3Id++;
      widget.imgUrlsProblemsPartThree.add(urlDownload);
    });
  }
