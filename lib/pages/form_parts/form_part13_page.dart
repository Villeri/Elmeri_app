...
import 'form_part14_page.dart';

class FormPartThirteenPage extends StatefulWidget {
  String currentRoom;
  ...
  @override
  State<FormPartThirteenPage> createState() => _FormPartThirteenPageState();
}

class _FormPartThirteenPageState extends State<FormPartThirteenPage> {
  int thingsOk13 = 0;
  int thingsNotOk13 = 0;
  TextEditingController problemThirteenController = TextEditingController();
      ...
      body: SingleChildScrollView(
        child: Center(
          child: Column(
            children: [
              FormCardWidget(
                headerText: "13. Lämpöolosuhteet",
                thingsOk: thingsOk13.toString(),
                thingsNotOk: thingsNotOk13.toString(),
                plusThingsOk: plusThingsOkPartThirteen,
                minusThingsOk: minusThingsOkPartThirteen,
                plusThingsNotOk: plusThingsNotOkPartThirteen,
                minusThingsNotOk: minusThingsNotOkPartThirteen,
              ),
                ...
                child: Column(
                  children: [
                    ...
                    ProblemCardProblemsWidget(
                      controller: problemThirteenController,
                    ),
                    const SizedBox(height: 15),
                    const Text(
                      "Vastuutahot:",
                          ...
                          ? () {
                              ...
                              setState(() {
                                problemsPartThirteen
                                    .add(problemThirteenController.text);
                                accountablePeoplePartThirteen
                                    .add(accountablePeoplePartThirteen);
                                urgenciesPartThirteen.add(currentUrgency);
                                problemCardsPartThirteen.add(
                                  Card(
                                    child: Column(
                                      children: [
                                        Text(
                                            "Poikkeama: ${problemThirteenController.text}",
                                            style:
              ...
              NextButton(onPressed: () {
                Navigator.push(
                    context,
                    MaterialPageRoute(
                        builder: (context) => FormPartFourteenPage(
                              ...
                              accountablePeoplePartThirteen:
                                  accountablePeoplePartThirteen,
                              imgUrlsPartThirteen: imgUrlsProblemsPartThirteen,
                              problemCardsPartThirteen:
                                  problemCardsPartThirteen,
                              problemsPartThirteen: problemsPartThirteen,
                              thingsNotOk13: thingsNotOk13,
                              thingsOk13: thingsOk13,
                              urgenciesPartThirteen: urgenciesPartThirteen,
                            )));
              })
            ],
          ),
  ...
  plusThingsOkPartThirteen() {
    setState(() {
      thingsOk13++;
    });
  }

  minusThingsOkPartThirteen() {
    if (thingsOk13 > 0) {
      setState(() {
        thingsOk13--;
      });
  ...
  plusThingsNotOkPartThirteen() {
    setState(() {
      thingsNotOk13++;
    });
  }

  minusThingsNotOkPartThirteen() {
    if (thingsNotOk13 > 0) {
      setState(() {
        thingsNotOk13--;
      });
  ...
  Future saveDataToDatabase() async {
    if (file == null) return;
    final fileName = "part13problem$problem13Id";
    final destination = "files/$fileName";
    _problemData.problem = problemThirteenController.text.trim();
    _problemData.accountablePeople = problemsAccountablePeople;
    _problemData.urgency = currentUrgency;
    _problemData.problemId = problem13Id.toString();
    ...
    setState(() {
      problem13Id++;
      imgUrlsProblemsPartThirteen.add(urlDownload);
    });
  }
