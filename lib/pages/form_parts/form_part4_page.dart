...
import 'form_part5_page.dart';

class FormPartFourPage extends StatefulWidget {
  String currentRoom;
  ...
  @override
  State<FormPartFourPage> createState() => _FormPartFourPageState();
}

class _FormPartFourPageState extends State<FormPartFourPage> {
  int thingsOk4 = 0;
  int thingsNotOk4 = 0;
  TextEditingController problemFourController = TextEditingController();
      ...
      body: SingleChildScrollView(
        child: Center(
          child: Column(
            children: [
              FormCardWidget(
                headerText: "4. Koneiden kunto ja suojalaitteet",
                thingsOk: thingsOk4.toString(),
                thingsNotOk: thingsNotOk4.toString(),
                plusThingsOk: plusthingsOkPartFour,
                minusThingsOk: minusthingsOkPartFour,
                plusThingsNotOk: plusThingsNotOkPartFour,
                minusThingsNotOk: minusThingsNotOkPartFour,
              ),
                ...
                child: Column(
                  children: [
                    ...
                    ProblemCardProblemsWidget(
                      controller: problemFourController,
                    ),
                    const SizedBox(height: 15),
                    const Text(
                      "Vastuutahot:",
                    ...
                    SaveProblemButtonWidget(
                      onPressed: file != null
                          ? () {
                              ...
                              setState(() {
                                problemsPartFour
                                    .add(problemFourController.text);
                                accountablePeoplePartFour
                                    .add(accountablePeoplePartFour);
                                urgenciesPartFour.add(currentUrgency);
                                problemCardsPartFour.add(
                                  Card(
                                    child: Column(
                                      children: [
                                        Text(
                                            "Poikkeama: ${problemFourController.text}",
                                            style:
              ...
              NextButton(onPressed: () {
                Navigator.push(
                    context,
                    MaterialPageRoute(
                        builder: (context) => FormPartFivePage(
                              ...
                              accountablePeoplePartFour:
                                  accountablePeoplePartFour,
                              imgUrlsPartFour: imgUrlsProblemsPartFour,
                              problemCardsPartFour: problemCardsPartFour,
  ...
  plusthingsOkPartFour() {
    setState(() {
      thingsOk4++;
    });
  }

  minusthingsOkPartFour() {
    if (thingsOk4 > 0) {
      setState(() {
        thingsOk4--;
      });
  ...
  plusThingsNotOkPartFour() {
    setState(() {
      thingsNotOk4++;
    });
  }

  minusThingsNotOkPartFour() {
    if (thingsNotOk4 > 0) {
      setState(() {
        thingsNotOk4--;
      });
  ...
  Future saveDataToDatabase() async {
    if (file == null) return;
    final fileName = "part4problem$problem4Id";
    final destination = "files/$fileName";
    _problemData.problem = problemFourController.text.trim();
    _problemData.accountablePeople = problemsAccountablePeople;
    _problemData.urgency = currentUrgency;
    _problemData.problemId = problem4Id.toString();
    ...
    setState(() {
      problem4Id++;
      imgUrlsProblemsPartFour.add(urlDownload);
    });
  }
